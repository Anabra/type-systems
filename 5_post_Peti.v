...
Proof.
  ...
  induction H; intros.
  - eapply C_variable.
    unfold VarSet.Subset in H0.
    exact (H0 v H).

  - eapply C_lambda.
    eapply (IHClosed' (v :: c2)).
    unfold VarSet.Subset.
    intros. simpl.
    destruct H1.
    + left. exact H1.
    + right. exact (H0 v0 H1).

  - eapply C_application.
    + exact (IHClosed'1 c2 H1).
    + exact (IHClosed'2 c2 H1).
Qed.
...
Proof.
  ...
  - simpl.
    ...
    + pose (neq := eqb_neq v v0).
      ...
      pose (eqf := H2 n).
      rewrite eqf.
      pose (nic := proj2 (not_in_cons v v0 c)).
      pose (nic1 := nic (conj n H0)).
      pose (ihr := IHClosed' nic1).
      rewrite ihr.
      reflexivity.

  - simpl.
    rewrite (IHClosed'1 H0).
    rewrite (IHClosed'2 H0).
    reflexivity.
Qed.
...
Proof.
  eapply (andTrue false); prove_closed.
Qed.
